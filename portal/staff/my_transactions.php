<?php
  ...
  if ($item_id) { $where .= " AND t.item_id = ?"; $params[] = $item_id; $types .= 'i'; }
  if ($type) { $where .= " AND t.type = ?"; $params[] = $type; $types .= 's'; }
  if ($date_from) { $where .= " AND DATE(t.timestamp) >= ?"; $params[] = $date_from; $types .= 's'; }
  if ($date_to) { $where .= " AND DATE(t.timestamp) <= ?"; $params[] = $date_to; $types .= 's'; }

  $stats_sql = "SELECT COUNT(*) as total_transactions,
  ...
  $sales_types = 'i';
  if ($date_from) { $sales_where .= " AND DATE(sale_date) >= ?"; $sales_params[] = $date_from; $sales_types .= 's'; }
  if ($date_to) { $sales_where .= " AND DATE(sale_date) <= ?"; $sales_params[] = $date_to; $sales_types .= 's'; }

  $sales_stats_sql = "SELECT COUNT(*) as total_sales, IFNULL(SUM(total_amount),0) as total_revenue FROM sales WHERE $sales_where";
  ...
  $sql = "SELECT t.id, t.timestamp, i.item_name, i.unit, t.quantity, t.type, t.remarks
          ...
          WHERE $where ORDER BY t.timestamp DESC LIMIT ? OFFSET ?";
...
<div class="container-fluid">
  <div class="row mb-4">
    <div class="col-12">
      <div class="d-flex justify-content-between align-items-center">
        <div>
          <h3 class="h3 mb-0" style="color: #382417;">My Activities</h3>
  ...
  <div class="row g-3 mb-4">
    <div class="col-xl-3 col-md-6">
      <div class="card text-white h-100" style="background: linear-gradient(135deg, #382417 0%, #2a1505 100%);">
        <div class="card-body text-center p-4">
          <div class="mb-3"><i class="fas fa-cash-register fa-2x opacity-75"></i></div>
    ...
    <div class="col-xl-3 col-md-6">
      <div class="card text-white h-100" style="background: linear-gradient(135deg, #6b3a1f 0%, #3d1c02 100%);">
        <div class="card-body text-center p-4">
          <div class="mb-3"><i class="fas fa-peso-sign fa-2x opacity-75"></i></div>
    ...
    <div class="col-xl-3 col-md-6">
      <div class="card text-white h-100" style="background: linear-gradient(135deg, #5a2d00 0%, #3d1c02 100%);">
        <div class="card-body text-center p-4">
          <div class="mb-3"><i class="fas fa-arrow-up fa-2x opacity-75"></i></div>
    ...
    <div class="col-xl-3 col-md-6">
      <div class="card text-white h-100" style="background: linear-gradient(135deg, #c87533 0%, #a05a20 100%);">
        <div class="card-body text-center p-4">
          <div class="mb-3"><i class="fas fa-arrow-down fa-2x opacity-75"></i></div>
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
          <h5 class="mb-0">
            <i class="fas fa-shopping-cart me-2 icon-brown"></i>My Sales History
          </h5>
        ...
        <div class="card-body p-0">
          ...
          $sales_list_types = 'i';
          if ($date_from) { $sales_list_where .= " AND DATE(sale_date) >= ?"; $sales_list_params[] = $date_from; $sales_list_types .= 's'; }
          if ($date_to) { $sales_list_where .= " AND DATE(sale_date) <= ?"; $sales_list_params[] = $date_to; $sales_list_types .= 's'; }
            ...
            <div class="table-responsive">
              <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                  ...
                  ?>
                  <tr>
                    <td><strong class="text-muted">#<?php echo str_pad($sale['id'],6,'0',STR_PAD_LEFT); ?></strong></td>
                    <td><i class="fas fa-user me-1 text-muted" style="font-size:0.75rem;"></i><?php echo htmlspecialchars($sale['customer_name'] ?: 'Walk-in'); ?></td>
                    <td class="text-end"><span class="fw-bold" style="color:#3b2008;">₱<?php echo number_format($sale['total_amount'],2); ?></span></td>
                    <td class="text-center">
                      <span class="badge bg-<?php echo $color; ?>">
                        <i class="fas <?php echo $icon; ?> me-1"></i><?php echo ucfirst($sale['payment_method']); ?>
                      </span>
                    </td>
                    <td class="text-muted small">
                      <div><i class="fas fa-calendar me-1"></i><?php echo date('M j, Y', strtotime($sale['sale_date'])); ?></div>
                      <div><i class="fas fa-clock me-1"></i><?php echo date('h:i A', strtotime($sale['sale_date'])); ?></div>
                    </td>
                  </tr>
                  <?php endwhile; ?>
                </tbody>
        ...
        <div class="card-body">
          ...
          <div class="mt-2 pt-2 border-top">
            ...
            <div class="d-flex flex-wrap gap-1">
              <?php if ($item_id): $item_name = array_filter($items, fn($i) => $i['id'] == $item_id)[0]['item_name'] ?? ''; ?>
                <span class="badge bg-secondary">Item: <?php echo htmlspecialchars($item_name); ?></span>
              <?php endif; ?>
              <?php if ($type): ?><span class="badge bg-secondary">Type: <?php echo ucfirst($type); ?></span><?php endif; ?>
              <?php if ($date_from): ?><span class="badge bg-secondary">From: <?php echo date('M j, Y', strtotime($date_from)); ?></span><?php endif; ?>
              <?php if ($date_to): ?><span class="badge bg-secondary">To: <?php echo date('M j, Y', strtotime($date_to)); ?></span><?php endif; ?>
            </div>
          </div>
          <?php endif; ?>
        </div>
  ...
  <div class="row">
    <div class="col-12">
      <div class="card">
        <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
          <h5 class="mb-0">
            <i class="fas fa-clipboard-list me-2 icon-brown"></i>Inventory Transaction Records
          </h5>
          <span class="badge bg-brown"><?php echo number_format($total_items); ?> Records</span>
        </div>
        <div class="card-body p-0">
          <?php if ($transactions->num_rows === 0): ?>
            <div class="text-center py-5">
              ...
              <?php if ($item_id || $type || $date_from || $date_to): ?>
                ...
                <a href="my_transactions.php" class="btn btn-outline-brown">Clear Filters</a>
              <?php else: ?>
                <p class="text-muted mb-3">You haven't made any inventory transactions yet</p>
                <a href="<?php echo getBaseURL(); ?>/staff_inventory.php" class="btn btn-brown">
          ...
          <?php else: ?>
            <div class="table-responsive">
              <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                  <?php $row_number = $offset + 1; while ($row = $transactions->fetch_assoc()): ?>
                  <tr>
                    <td class="text-center text-muted small"><strong><?php echo $row_number++; ?></strong></td>
                    <td>
                    ...
                    <td class="text-center">
                      <strong class="fs-5 icon-brown">
                        <?php echo $row['type'] === 'stock-in' ? '+' : '-'; ?><?php echo number_format($row['quantity']); ?>
                      </strong>
                    </td>
                    <td class="text-center">
                      <?php if ($row['type'] === 'stock-in'): ?>
                        <span class="badge bg-brown-light"><i class="fas fa-arrow-up me-1"></i>Stock In</span>
                      <?php else: ?>
                        <span class="badge bg-brown-dark"><i class="fas fa-arrow-down me-1"></i>Stock Out</span>
                      <?php endif; ?>
                    </td>
                    <td>
                      <?php $remarks = $row['remarks']; if (strlen($remarks) > 50): ?>
                    ...
                    <td class="text-muted small">
                      <div><i class="fas fa-calendar me-1" style="color:#3b2008;"></i><?php echo date('M j, Y', strtotime($row['timestamp'])); ?></div>
                      <div><i class="fas fa-clock me-1"></i><?php echo date('h:i A', strtotime($row['timestamp'])); ?></div>
                    </td>
                  </tr>
                  <?php endwhile; ?>
                </tbody>
            ...
            <div class="col-md-6">
              <nav aria-label="Page navigation">
                <ul class="pagination pagination-sm justify-content-md-end justify-content-center mb-0">
                  <?php if ($page > 1): ?>
                  <li class="page-item">
                    <a class="page-link" href="?page=1&item_id=<?php echo $item_id; ?>&type=<?php echo urlencode($type); ?>&date_from=<?php echo urlencode($date_from); ?>&date_to=<?php echo urlencode($date_to); ?>"><i class="fas fa-angle-double-left"></i></a>
                  </li>
                  <?php endif; ?>
                  <li class="page-item <?php echo $page<=1?'disabled':''; ?>">
                    <a class="page-link" href="?page=<?php echo max(1,$page-1); ?>&item_id=<?php echo $item_id; ?>&type=<?php echo urlencode($type); ?>&date_from=<?php echo urlencode($date_from); ?>&date_to=<?php echo urlencode($date_to); ?>"><i class="fas fa-angle-left"></i></a>
                  </li>
                  <?php
                  $start_page = max(1,$page-2); $end_page = min($total_pages,$page+2);
                  if ($start_page > 1) { echo '<li class="page-item"><a class="page-link" href="?page=1&item_id='.$item_id.'&type='.urlencode($type).'&date_from='.urlencode($date_from).'&date_to='.urlencode($date_to).'">1</a></li>'; if ($start_page>2) echo '<li class="page-item disabled"><span class="page-link">...</span></li>'; }
                  for ($i=$start_page;$i<=$end_page;$i++): ?>
                    <li class="page-item <?php echo $i==$page?'active':''; ?>">
                      <a class="page-link" href="?page=<?php echo $i; ?>&item_id=<?php echo $item_id; ?>&type=<?php echo urlencode($type); ?>&date_from=<?php echo urlencode($date_from); ?>&date_to=<?php echo urlencode($date_to); ?>"><?php echo $i; ?></a>
                    </li>
                  <?php endfor;
                  if ($end_page < $total_pages) { if ($end_page<$total_pages-1) echo '<li class="page-item disabled"><span class="page-link">...</span></li>'; echo '<li class="page-item"><a class="page-link" href="?page='.$total_pages.'&item_id='.$item_id.'&type='.urlencode($type).'&date_from='.urlencode($date_from).'&date_to='.urlencode($date_to).'">'.$total_pages.'</a></li>'; }
                  ?>
                  <li class="page-item <?php echo $page>=$total_pages?'disabled':''; ?>">
                    <a class="page-link" href="?page=<?php echo min($total_pages,$page+1); ?>&item_id=<?php echo $item_id; ?>&type=<?php echo urlencode($type); ?>&date_from=<?php echo urlencode($date_from); ?>&date_to=<?php echo urlencode($date_to); ?>"><i class="fas fa-angle-right"></i></a>
                  </li>
                  <?php if ($page < $total_pages): ?>
                  <li class="page-item">
                    <a class="page-link" href="?page=<?php echo $total_pages; ?>&item_id=<?php echo $item_id; ?>&type=<?php echo urlencode($type); ?>&date_from=<?php echo urlencode($date_from); ?>&date_to=<?php echo urlencode($date_to); ?>"><i class="fas fa-angle-double-right"></i></a>
                  </li>
                  <?php endif; ?>
                </ul>
              </nav>
  ...
  <!-- My Activity Overview box -->
  <div class="row mt-4">
    <div class="col-12">
      <div class="alert-brown-overview">
...
.pagination .page-link { color: #6c757d; border-radius: 0.25rem; margin: 0 2px; transition: all 0.2s; }
.pagination .page-link:hover { background-color: #f8f9fa; border-color: #dee2e6; color: #382417; }
.pagination .page-item.active .page-link { background-color: #382417; border-color: #382417; color: white; font-weight: 600; }
.pagination .page-item.disabled .page-link { color: #adb5bd; background-color: transparent; border-color: #dee2e6; }

/* My Activity Overview box — brown themed */
.alert-brown-overview {
